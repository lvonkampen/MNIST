...
def split_speakers(audio_dir, train_perc=0.7, val_perc=0.2):
    speakers = set()
...
def main():
    ...
    test_loss = evaluate(model, test_loader, Hyperparameters.loss_func)
    print(f"Test Loss: {test_loss:.6f}")

    torch.save(model.state_dict(), "WhisperVIAModel_NEW.pth")
    print("Model saved as 'WhisperVIAModel_NEW.pth'")

if __name__ == "__main__":
    main()

# make sure split_speakers stores the randomized speakers for future uses of the data

# start error analysis - find where the error is the largest to mitigate the problem potentially
# try adding sigmoid as the activation instead - Sigmoid uses 0.0 - 1.0 while ReLU uses anything
# make sure to implement recall / F1 /
# summarize using inference -- take video, hyperparameters, and segments and output a new lecture video with 5 minutes worth of the most relevant segments (in descending order)
